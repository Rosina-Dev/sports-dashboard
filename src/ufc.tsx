...
function App() {
  ...
  const filteredEvents = events.filter((event) => {
    const text = `${event.name} ${event.type} ${event.city} ${event.venue} ${event.tagline} ${event.note} ${event.fights.map((fight) => `${fight.red} ${fight.blue} ${fight.division} ${fight.priority} ${fight.impact} ${fight.stakes} ${fight.why} ${fight.alerts?.join(" ") ?? ""}`).join(" ")}`;
    return (eventType === "All" || event.type === eventType) && event.fights.some((fight) => matchesDivision(fight, division)) && includesQuery(text, normalizedQuery);
  });
  ...
  const fights = selected.fights.filter((fight) => matchesDivision(fight, division) && includesQuery(`${fight.red} ${fight.blue} ${fight.division} ${fight.tier} ${fight.priority} ${fight.stakes} ${fight.impact} ${fight.why} ${fight.alerts?.join(" ") ?? ""}`, normalizedQuery));
  ...
  const searchMatchCount = filteredEvents.length + fights.length + activeNews.length + activeRankings.length + watchlist.length + activeStories.length + activeResults.length + alertItems.length;
