...
function App() {
  ...
  const liveStatusLabel = liveOkCount === liveSnapshot.sources.length ? "Official sources checked" : liveOkCount > 0 ? "Partial source check" : "Scheduled source check pending";
  ...
  return <main className="shell">
    ...
    <section className="noticeBar cardSourceNote"><Radio size={18} /><span><strong>{selected.name}</strong>: card-specific tabs use curated bout data and should be verified against <a href={selected.sourceUrl} target="_blank" rel="noreferrer">{selected.sourceLabel}</a> before fight night.</span></section>

    <section className="noticeBar sourceCheck"><Radio size={18} /><span>{lastUpdated}. Tabs above are selected-card context. News and rankings below are general UFC pulse unless labeled otherwise.</span></section>

    <section className="panel hybridPanel"><div className="feedHeader"><div><p className="eyebrow">Official source status</p><h2>{liveStatusLabel}</h2></div><span className="snapshotTime">Checked {formatDateTime(liveSnapshot.generatedAt)}</span></div><div className="sourceGrid">{liveSnapshot.sources.map((source) => <a className="sourceCard" href={source.url} target="_blank" rel="noreferrer" key={source.id}><span className={`sourceState source-${source.status}`}>{source.status === "seeded" ? "pending" : source.status}</span><strong>{source.label}</strong><p>{source.summary}</p><small>{source.nextAction}</small></a>)}</div><p className="hybridNote">This is not the card-news feed. It only shows whether official UFC source pages have been checked. The dashboard still keeps curated card analysis separate.</p></section>

    <section className="pulseGrid" aria-label="general UFC news rankings and alerts pulse">
      <article className="pulseCard urgentPulse"><div className="panelTitle"><Newspaper size={19} /><h2>General UFC News</h2></div>{activeNews.slice(0, 2).length ? activeNews.slice(0, 2).map((item) => <a href={item.url} target="_blank" rel="noreferrer" key={item.title}><span>{item.tone}</span><strong>{item.title}</strong><p>{item.summary}</p></a>) : <EmptyState title="No news matches" note="Try a fighter, event, division, or alert phrase." />}</article>
      <article className="pulseCard"><div className="panelTitle"><Trophy size={19} /><h2>General UFC Rankings</h2></div>{activeRankings.slice(0, 3).length ? activeRankings.slice(0, 3).map((ranking) => <div key={ranking.division}><span>{ranking.state}</span><strong>{ranking.division}</strong><p>{ranking.movement}</p></div>) : <EmptyState title="No rankings match" note="Clear search or change division." />}</article>
