...
const focusModes = ["Top fights", "News", "Rankings", "Alerts", "Prospects"] as const;
...
function App() {
  ...
  const focusFights = searchedFights.filter((fight) => {
    if (focusMode === "Prospects") return fight.priority === "Prospect watch";
    if (focusMode === "Alerts") return fight.alerts?.length || fight.late || fight.priority === "Volatility watch";
    if (focusMode === "Top fights") return topPriorities.includes(fight.priority);
    return true;
  });
  const fights = focusFights.length || focusMode !== "Top fights" ? focusFights : searchedFights;
  ...
  const quickNews = activeNews.slice(0, 2);
  const quickRankings = activeRankings.slice(0, 3);
  const quickAlerts = alertItems.slice(0, 3);
  ...
  return <main className="shell">
    <section className="hero compactHero" style={{ backgroundImage: `linear-gradient(90deg, rgba(6,6,8,.94), rgba(20,22,28,.62)), url(${selected.image})` }}>
      <div className="heroCopy">
        <p className="eyebrow">UFC Fan Command</p>
        <h1>{selected.name}</h1>
        <p className="intro">A mobile fight feed for card context, rankings movement, alerts, and the updates he would otherwise chase on TikTok.</p>
        <div className="sourceRail"><a href={links.events} target="_blank" rel="noreferrer">UFC Events</a><a href={links.news} target="_blank" rel="noreferrer">UFC News</a><a href={links.rankings} target="_blank" rel="noreferrer">UFC Rankings</a></div>
      </div>
    ...
    <section className="toolbar mobileToolbar">
      <label className="searchBox"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search fighter, ranking, alert..." /></label>
    </section>

    <nav className="focusTabs" aria-label="UFC dashboard focus">
      {focusModes.map((mode) => <button className={focusMode === mode ? "selected" : ""} key={mode} onClick={() => setFocusMode(mode)}>{mode}</button>)}
    </nav>

    <section className="cardSwitcher" aria-label="Change card">
      <div><span>Change card</span><strong>{formatDate(selected.date)} - {selected.city}</strong></div>
      <div>{events.map((event) => <button className={selected.id === event.id ? "selected" : ""} key={event.id} onClick={() => setSelectedId(event.id)}>{event.name.replace("UFC ", "")}</button>)}</div>
    </section>
    ...
    <section className="pulseGrid" aria-label="news rankings and alerts pulse">
      <article className="pulseCard urgentPulse"><div className="panelTitle"><Newspaper size={19} /><h2>Latest Intel</h2></div>{quickNews.length ? quickNews.map((item) => <a href={item.url} target="_blank" rel="noreferrer" key={item.title}><span>{item.tone}</span><strong>{item.title}</strong><p>{item.summary}</p></a>) : <EmptyState title="No news matches" note="Try a fighter, event, division, or alert phrase." />}</article>
      <article className="pulseCard"><div className="panelTitle"><Trophy size={19} /><h2>Ranking Pulse</h2></div>{quickRankings.length ? quickRankings.map((ranking) => <div key={ranking.division}><span>{ranking.state}</span><strong>{ranking.division}</strong><p>{ranking.movement}</p></div>) : <EmptyState title="No rankings match" note="Clear search or change division." />}</article>
      <article className="pulseCard"><div className="panelTitle"><AlertTriangle size={19} /><h2>What Changed</h2></div>{quickAlerts.length ? quickAlerts.map(({ event, fight }) => <div key={`${event.id}-${fight.id}`}><span>{fight.priority}</span><strong>{fight.red} vs {fight.blue}</strong><p>{fight.alerts?.[0] ?? fight.why}</p></div>) : <EmptyState title="No alert matches" note="Try replacement, TBD, backup, or prospect." />}</article>
    </section>

    <section className="eventSpotlight feedSpotlight"><div className="spotlightBody"><div className="sectionLabel"><span>{selected.type}</span><span>{heatLabel}</span></div><h2>{fightText(main)}</h2><p>{main?.why ?? selected.tagline}</p><div className="eventFacts"><span><CalendarDays size={16} />{formatDate(selected.date)}</span><span><Activity size={16} />{selected.venue}</span><span><Clock3 size={16} />{selected.watch}</span></div><div className="mainFights"><div><small>Main event</small><strong>{fightText(main)}</strong><span>{main?.stakes ?? "Official listing pending"}</span></div><div><small>Next priority</small><strong>{fightText(coMain)}</strong><span>{coMain?.stakes ?? "Official listing pending"}</span></div></div><div className="linkRow"><a href={selected.sourceUrl} target="_blank" rel="noreferrer">{selected.sourceLabel}</a><a href={selected.url} target="_blank" rel="noreferrer">Official UFC check</a></div></div></section>

    <section className="panel fightCardPanel feedPanel"><div className="feedHeader"><div><p className="eyebrow">{focusMode}</p><h2>Fight Feed</h2></div><select value={division} onChange={(event) => setDivision(event.target.value as Division)} aria-label="Filter fights by division">{divisions.map((option) => <option key={option} value={option}>{option === "All" ? "All divisions" : option}</option>)}</select></div><div className="fightList">{fights.length ? fights.map((fight) => <div className="fightRow" key={fight.id}><div className="fightTier"><span>{fight.tier}</span><small>{fight.division}</small><b className="priority">{fight.priority}</b></div><div className="fighters"><strong>{fight.red}</strong><span>vs</span><strong>{fight.blue}</strong></div><div className="fightNotes"><p>{fight.stakes}</p><small>{fight.impact}</small><em>{fight.why}</em>{fight.alerts?.length ? <div className="alertStrip">{fight.alerts.map((alert) => <span key={alert}>{alert}</span>)}</div> : null}</div><div className="fightFlags">{fight.title ? <span>Title stakes</span> : null}{fight.rivalry ? <span>Rivalry</span> : null}{fight.late ? <span>Card-change risk</span> : null}{!fight.title && !fight.rivalry && !fight.late ? <span>{fight.ranked} ranked/name signal(s)</span> : null}</div></div>) : <EmptyState title="No fights match" note="Change the focus, division, or search term." />}</div></section>
